...
if [[ -z "$upstream_version" ]]; then
  echo "ERREUR : impossible d'extraire la version depuis package.json amont" >&2
  exit 1
fi
if [[ -z "$upstream_description" ]]; then
  echo "ERREUR : impossible d'extraire la description depuis package.json amont" >&2
  exit 1
fi
echo "Version amont     : $upstream_version"
echo "Description amont : $upstream_description"
...
if ! grep -qE '"version"[[:space:]]*:[[:space:]]*"[^"]*"' "$plugin_json"; then
  echo "ERREUR : champ \"version\" introuvable dans plugin.json" >&2
  exit 1
fi
if ! grep -qE '"description"[[:space:]]*:[[:space:]]*"' "$plugin_json"; then
  echo "ERREUR : champ \"description\" introuvable dans plugin.json" >&2
  exit 1
fi
...
echo
echo "Sync terminee."
echo "  Ref amont           : $UPSTREAM_REF"
echo "  SHA amont importe   : $upstream_sha"
echo "  Skills synchronises : $count"
echo "  Version plugin.json : $upstream_version"
echo "  Description         : $upstream_description"
echo
